...
import { ExerciseSetHistory } from "./ExerciseSetHistory";
...
export function ExerciseCard({
  ...
  // Only logged exercises can reveal their per-set breakdown.
  const canExpand = isCompleted && hasLogs;

  return (
    <div className="flex flex-col rounded-[14px] bg-[#1B1E2B] p-3 shadow-xl ring-1 ring-white/5">
      <div
        className="group flex w-full cursor-pointer items-center gap-5 text-left"
        role="button"
        tabIndex={0}
        onClick={onCardClick}
      >
      ...
      {canExpand && (
        <>
          <button
            ...
            aria-label={t("exerciseCard.toggleHistory")}
            className="mt-2 flex w-full items-center justify-center border-t border-white/5 pt-2 text-slate-400 transition-colors hover:text-slate-200"
          >
            <ChevronDownIcon
          ...
          >
            <div className="overflow-hidden">
              <ExerciseSetHistory exercise={exercise} loggedSets={loggedSets!} />
            </div>
          </div>
        </>
      )}
