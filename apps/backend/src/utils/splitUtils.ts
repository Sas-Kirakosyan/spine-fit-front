export const SPLIT_TARGET_MUSCLES: Record<string, string[]> = {
  ...
  UPPER_LOWER_ALTERNATING: ["chest", "lats", "upper_back", "front_delts", "rear_delts", "triceps", "biceps", "quads", "glutes", "hamstrings"],
...
export function mapSplitType(split: string): string {
  ...
  if (/Upper.*Lower.*Alternating/i.test(split)) return "UPPER_LOWER_ALTERNATING";
