...
describe("enforceFullBodyABRequirements", () => {
  test("adds missing Day B leg compound and keeps vertical pull on Day B", () => {
    ...
    const legExtension = ex(407, "Leg Extension", ["quads"]);
    ...
    const legPress = ex(24, "Leg Press", ["quads", "glutes", "hamstrings"]);

    const workoutDays = [
      {
        dayNumber: 0,
        dayName: "Full Body",
        muscleGroups: ["chest", "lats", "upper_back", "quads", "glutes", "hamstrings"],
